...
fn process_file(
    ...
    whole_document: bool,
    whole_paragraphs: bool,
) -> Result<Vec<(f64, PathBuf)>, io::Error> {
// ) -> Result<f64, io::Error> {
    ...
    let text = match fs::read_to_string(input_file) {
        ...
        Err(e) => {
            ...
            let v = vec![(-1.0, input_file.clone())];
            return Ok(v);
        }
     };
    // match text {
    ...
    for _ in 0..1 {
        ...
        overlaps.push((overlap, input_file.clone()))
    }
...
fn main() {
    ...
    let outputs: Vec<Vec<(f64, PathBuf)>> = rx.lock().unwrap().iter().collect();
    // println!("Collected outputs: {:?}", outputs);

    let result_file = format!("./overlaps_e{}_s{}_n{}-{}_name_{}",
