...
# Code from https://github.com/pratyushmaini/llm_dataset_inference
...
def get_aggregate_metrics(model, tokenizer, dataset, metric_list, args, batch_size = 32):
    '''
    ...
    if "10_min_probs" in metric_list:
        for k in [0.1]:
            metrics[f"k_min_probs_{k}"] = k_min_probs(loss_list, k=k)

    if "10_max_probs" in metric_list:
        for k in [0.1]:
